...
async def test_approve_flight_plan(client: TestClient, scheduling_plugin: Scheduling):
    ...
    existing_plan = FlightPlan(
        ...
        flight_plan=FlightPlanDetail(
            name="Plan to Approve", body=[{"name": "do-something"}]
        ),
    ...
    with (
        patch.object(
            scheduling_plugin,
            "_Scheduling__get_flight_plan",
            return_value=existing_plan,
        ) as mock_get_plan,
        patch.object(
            scheduling_plugin, "_Scheduling__update_approval", return_value=None
        ) as mock_update_approval,
        patch(
        ...
        mock_get_plan.assert_awaited_once_with(
            flight_plan_uuid=fp_uuid, user_id=user_id
        )
