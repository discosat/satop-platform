...
def scheduling_plugin(tmp_path, mock_app):
    ...
    (tmp_path / "config.yaml").write_text(yaml.dump({
        'name': 'Scheduling_Test', 'capabilities': ['http.add_routes']
    }))
    plugin = Scheduling(
...
def test_save_new_flightplan(client: TestClient, scheduling_plugin: Scheduling):
    ...
    scheduling_plugin.app.syslog.create_artifact.return_value = MagicMock(sha1="mock_artifact_sha1")
    flight_plan_data = {
...
async def test_approve_flight_plan(client: TestClient, scheduling_plugin: Scheduling):
    ...
    with patch("satop_plugins.flight_planning.scheduling.BackgroundTasks.add_task") as mock_add_task:
        response = client.post(f"/approve/{fp_uuid}?approved=true")
        assert response.status_code == 202
        assert response.json()["message"] == "Flight plan approved and scheduled for transmission to ground station."
