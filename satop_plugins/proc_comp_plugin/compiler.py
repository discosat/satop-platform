...
class Compiler(Plugin):
    def __init__(self, *args, **kwargs):
        ...
        async def new_compile(
            flight_plan_instructions: FlightPlanInstructions, request: Request
        ) -> CompiledInstructions:
            comiled_plan, compiled_artifact_id = await self.compile(
                flight_plan=flight_plan_instructions.instructions,
                user_id=request.state.userid,
            )

            return CompiledInstructions(
                instructions=comiled_plan, artifact_id=compiled_artifact_id
            )
    ...
    async def compile(self, flight_plan: dict, user_id: str):
        ...
        try:
            artifact_in_id = self.sys_log.create_artifact(
                flight_plan_as_bytes, filename="flight_plan.json"
            ).sha1
            ...
            logger.info(
                f"Received existing flight plan with artifact ID: {artifact_in_id}"
            )
        ...
        try:
            artifact_out_id = self.sys_log.create_artifact(
                io.BytesIO(compiled_as_bytes), filename="flight_plan.csh"
            ).sha1
        except sqlalchemy.exc.IntegrityError as e:
            # Artifact already exists
        ...
        logger.info(
            f"Compiled flight_plan with ID: {artifact_in_id} into CSH with ID: {artifact_out_id}"
        )
