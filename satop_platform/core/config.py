...
def get_root_data_folder():
    ...
    if env_path:
        ...
        try:
            ...
            logger.warning(
                f"Data directory defined in env ({env_path}) does not exist and could not be created. Using OS specific path. Error: {e}"
            )
...
class SatopConfig:
    ...
    def _get_config_file_name(
        self, parent_dir: pathlib.Path, config_name: str
    ) -> pathlib.Path | None:
        paths = list(
            chain(
        ...
        if len(paths) < 1:
            ...
            if len(paths) > 1:
                logger.warning(
                    f"Found multiple valid config files. Using the first: {paths[0]}"
                )
            return paths[0]
    ...
    def _traverse_config(self, config: dict | None, key_path: Iterable[str]):
        ...
        if not isinstance(item, dict):
            logger.error(
                f'Error traversing config for value "{key_path}" at "{next}" ({type(item)})'
            )
            raise LookupError
    ...
    def reload(self):
        default_config_dir = (
            pathlib.Path(satop_platform.__file__).parent.resolve() / "default/config"
        )
        user_config_dir = get_root_data_folder() / "config"

        default_config_path = self._get_config_file_name(
            default_config_dir, self.__config_name
        )
        user_config_path = self._get_config_file_name(
            user_config_dir, self.__config_name
        )
        ...
        if self.__default_config is None and self.__user_config is None:
            logger.warning(
                f'Config loader for "{self.__config_name}" found no config files.'
            )
