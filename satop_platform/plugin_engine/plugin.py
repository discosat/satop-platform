...
class Plugin:
    ...
    def call_function(self, plugin_name: str, func_name: str, *args, **kwargs):
        ...
        return self.app.plugin_engine.call_plugin_method(
            plugin_name, func_name, *args, **kwargs
        )
    ...
    def check_required_capabilities(self, required: Iterable[str]):
        ...
        if not check:
            self.logger.error(
                f'Plugin "{self.name}" does not have the required capabilities: {reqs - caps}'
            )
        return check
...
class AuthenticationProviderPlugin(Plugin):
    def create_token_pair(
        self, user_id: str | None = None, uuid: UUID | None = None
    ) -> TokenPair:
