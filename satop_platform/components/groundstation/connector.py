...
class GroundstationRegistrationItem:
    ...
    websocket: WebSocket
    busy: bool = False
    out_queue: Queue[tuple[UUID, dict|FramedContent, ProxyHeader|None]] = field(default_factory=Queue)
...
class GroundstationConnector:
    ...
    async def __websocket_send(self, gs_id:UUID, data:dict|FramedContent, proxy_header:ProxyHeader|None=None):
        request_id = uuid4()
    ...
    def __setup_routes(self, api: APIApplication):
        ...
        async def ws_connect_gs(websocket: WebSocket):
            ...
            if gs_id:
                ...
                logger.warning(f"GS Client token missing 'sub':")
                ...
                while True:
                    ...
                    match content:
                        ...
                        case FramedContent():
                            ...
                            if proxy_header:
                                header['proxy_header'] = dataclasses.asdict(proxy_header)
        ...
        async def ws_connect_gs_term(websocket:WebSocket, gs_id:UUID, term_id:str):
            ...
            if user_id:
                ...
                logger.warning(f"Terminal Client token missing 'sub':")
                await websocket.send_json({'error': 'token missing sub'})
                await websocket.close(code=1002)
                return
            
            reg_key = (gs_id, term_id)
            if not reg_key in self.registered_terminals:
                logger.warning(f"Terminal Client attempted connecting to non-existing terminal")
        ...
        async def control_groundstation_framed(gs_uuid: UUID, header_data:dict, request: Request):
            frames = header_data.pop('frames')
            await self.send_control(request, gs_uuid, FramedContent(header_data=header_data,frames=frames))
        ...
        async def get_all_terminals(request: Request):
            ...
            for (gs, tid),item in self.registered_terminals.items():
                terms.append({
                    ...
                    'ws_url': self.app.api.api_app.url_path_for('ws_connect_gs_term', gs_id=gs, term_id=tid)
                })
    ...
    async def send_control(self, request: Request, gs_uuid: UUID, data: dict|FramedContent):
        # TODO: Create proxy header from request
        proxy_header = ProxyHeader('control_frame',  request.state.token_payload.get('sub'))
        return await self.send_to_gs(gs_uuid, data, proxy_header)
        
    
    async def send_to_gs(self, gs_uuid: UUID, data: dict|FramedContent, proxy_header:ProxyHeader):
    ...
    async def handle_read_message_from_gs(self, gs_id:UUID, message):
        ...
        if msg_type is None:
            logger.warning(f'Message from groundstation missing type')
        ...
        logger.warning(f'Unsupported message type received from groundstation')
    ...
    async def handle_terminal_messages(self, gs_id:UUID, message: dict, command: str):
        ...
        match command:
            case 'open':
                ...
                self.registered_terminals[reg_key] = TerminalRegistrationItem(term_name, read_only=ro)
            ...
            case 'stdout':
                ...
                if not term:
                    logger.warning(f'Terminal {reg_key} does not exist. Cannot send response to clients')
                    return
