...
class Syslog:
            ...
            responses={
                status.HTTP_200_OK: {
                    ...
                    "content": {
                        "application/json": {
                            "example": {
                                "detail": "Artifact already exists. Reupload not neccessary. 5372ef2198557450d7424ba9c36151d932fb45f0"
                            }
                        }
                    },
                }
        ...
        def upload_artifact(file: UploadFile):
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"Artifact already exists. Reupload not neccessary. {e.params[0]}",
                )
    ...
    def log_event(self, event: models.Event):
        ...
        triples.append(
            models.Triple(
                subject=action,
                predicate=models.Predicate(descriptor="loggedAt"),
                object=ts,
            )
        )
        ...
        for r in rs:
            match r:
                case models.EventSubjectRelationship():
                    triples.append(
                        models.Triple(
                            subject=r.subject, predicate=r.predicate, object=action
                        )
                    )
                case models.EventObjectRelationship():
                    triples.append(
                        models.Triple(
                            subject=action, predicate=r.predicate, object=r.object
                        )
                    )
                case models.Triple():
                    triples.append(r)
    ...
    def get_artifact(self, hash):
        with sqlmodel.Session(self.db) as session:
            statement = sqlmodel.select(models.ArtifactStore).where(
                models.ArtifactStore.sha1 == hash
            )
